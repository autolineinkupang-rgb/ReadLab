...
function WriterDashboard() {
  ...
  async function fetchNovels(p: number) {
    ...
    setPage(p);
    try {
      const res = await novels.list({ page: p, limit: 20, sort: "created_at", order: "desc", writer_id: user!.id });
      setData(res.data || []);
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      {showAddForm && (
        <Card className="mb-6 p-4 space-y-3">
          <h2 className="text-white text-sm font-semibold">Create New Novel</h2>
          <input
            value={addForm.title}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <p className="text-sm text-gray-500 mb-4">You haven&apos;t created any novels yet.</p>
          <button
              ...
              <Card key={n.ID}>
                {editingId === n.ID ? (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                    <Link href={`/en/novel/${n.ID}/${n.Slug}`} className="flex-1 min-w-0 group">
                      <h3 className="text-sm font-semibold text-white group-hover:text-accent transition-colors truncate">{n.Title}</h3>
