      ...
      {data[0] && (
        <Card className="mt-8 p-6">
          ...
          <div className="flex gap-4">
            <div className="w-28 aspect-[3/4] rounded-lg bg-card-hover border border-line-light flex-shrink-0 flex items-center justify-center overflow-hidden">
              {data[0].CoverURL ? (
                <img src={data[0].CoverURL} alt="" className="w-full h-full object-cover" />
              ) : (
            ...
            <div className="min-w-0 flex-1">
              <Link href={`/en/novel/${data[0].ID}/${data[0].Slug}`} className="text-base font-semibold text-white hover:text-violet-400 transition-colors">
                {data[0].Title}
              </Link>
              <div className="flex items-center gap-3 mt-1 text-sm text-gray-400">
                <span>★ {data[0].Rating}</span>
                <span>📚 {data[0].Chapters}</span>
              </div>
              <p className="text-sm text-gray-400 mt-2 line-clamp-2">{data[0].Description}</p>
              <Link
                href={`/en/novel/${data[0].ID}/${data[0].Slug}/continue`}
